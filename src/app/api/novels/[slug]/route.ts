...
export async function GET(
    request: NextRequest,
    // ✨ [แก้ไข] แก้ไข Signature ของฟังก์ชัน GET ให้ถูกต้องตามหลัก Next.js App Router
    // argument ตัวที่สองคือ object `context` ที่มี `params` อยู่ข้างใน
    context: { params: { slug: string } }
) {
    // ✨ [แก้ไข] ดึงค่า slug ออกมาจาก context.params แทน
    ...
    try {
        ...
        // 1. รับ slug จาก `context.params` ที่ Next.js ถอดรหัสมาให้แล้ว
        const rawSlug = slug;
        ...
        // 2. ใช้ decodeURIComponent เพื่อความปลอดภัยเผื่อมีกรณีที่ยังไม่ได้ถอดรหัส
        ...
        // 3. ปรับปรุง Error Logging ให้แสดง slug ที่มีปัญหาได้ชัดเจนขึ้น
        const slugForError = (slug || 'unknown').substring(0, 100);
