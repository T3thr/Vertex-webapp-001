            ...
            {item.statuses.map((status: string, index: number) => (
              <span
                ...
                className={`px-2 py-1 text-xs rounded-full ${
                  status === 'reading' ? 'bg-blue-100 text-blue-700' :
                  status === 'owned' ? 'bg-green-100 text-green-700' :
                  status === 'following' ? 'bg-purple-100 text-purple-700' :
                  status === 'finished_reading' ? 'bg-gray-100 text-gray-700' :
                  'bg-gray-100 text-gray-600'
                }`}
              >
                {status === 'reading' ? 'กำลังอ่าน' :
                 status === 'owned' ? 'เป็นเจ้าของ' :
                 status === 'following' ? 'ติดตาม' :
                 status === 'finished_reading' ? 'อ่านจบแล้ว' :
                 status}
              </span>
            ))}
          </div>
